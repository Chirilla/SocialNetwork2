...
def check_result_KCG(C, Y, A, N, K, run_time):
    ...
    for k in range(K):
        ...
        x=((np.abs(A)-A)/2.0)
        print_negativeOut_edges(x, k, n, C, N)
        print(' ')
        print('Gli archi negativi interni del cluster sono:')
        print_negativeIn_edges(x,n)
        print(' ')
        print('Gli archi positivi che collegano il cluster agli altri cluster sono: ')
        y=((np.abs(A)+A)/2.0)
        print_positiveOut_edges(y,C,n,k)
        print(' ')
...
def print_negativeIn_edges(x,n):
  cx=sp.coo_matrix(x)
...
def print_positiveOut_edges(x, C ,elem , k):
  cx=sp.coo_matrix(x)
  for i,j,v in zip (cx.row, cx.col, cx.data):
    #if(v!=1):
    for m in elem:
      #print(str(C[j])+'  '+str(k))
      if(i==m and C[j]!=k+1 and C[j]!=-1 ):
        print('[('+str(i)+','+str(j)+'), cluster: '+str(C[j])+']',end=' ')


def print_negativeOut_edges(x, k, nodes, C, N):
  cx=sp.coo_matrix(x)
  for i,j,v in zip(cx.row, cx.col, cx.data):
    for m in nodes:
      for n in range(N):
        if(i==m and C[n]!=-1 and j==n):
